...
async def advanced_fuzzy_search_qa(
    ...
    scores_title = process.cdist([query_norm], df['title'].tolist(), scorer=fuzz.token_set_ratio)[0]
    ...
    # Get the top 60 results (3 pages, 20 items each)
    top_results = search_results.head(60)

    # Convert the filtered results to a dictionary
    records = top_results.to_dict(orient='records')

    # Paginate the results, each page contains 20 items
